...
glossary 是项目的错题本：预览编辑器保存时自动学习高置信错词写入其中，下次转录即自动纠正。仓库附带示例 `glossary.json` / `hotwords.json`（项目根），可直接编辑维护；个人词表不想提交的话，可自行把它们加回 `.gitignore`。

## 使用

仓库根附带示例视频 `20mb.mp4`（3.5 分钟，480×270），可直接用它试跑以下全流程。
