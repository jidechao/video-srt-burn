#!/usr/bin/env python3
"""Standalone burn stage — render reviewed subtitles into the final video.

Run this AFTER the manual preview save. It converts the saved transcript
(<video>.subtitle-work/subtitle-transcript.json by default) into SRT/ASS and
burns everything into an MP4 in one ffmpeg pass:

    python -m videotrans.burn <video.mp4> [options]

Ported from the oil-subtitle skill's burn_subtitles.py. Windows adaptations:
progress file under tempfile.gettempdir(), ffmpeg/ffprobe via shutil.which,
plain-ASCII log lines, and the macOS-Vision beauty pass degrades to a skip.
"""
...
from videotrans.config import resolve_glossary_path, resolve_progress_enabled
from videotrans.pipeline import default_work_dir
...
def set_display_replacements(entries: list[dict]):
    ...
    tokens, LLM echo, CJK/Latin spacing), so "GPT55" must also match
    "GPT 55" and "cloudcall" must match "cloud call".
    """
...
def load_user_glossary(override: str | Path | None = None) -> list[dict]:
    """Load user-specific subtitle corrections from the project glossary."""
...
def _subtitle_style_metrics(video_width: int, video_height: int) -> tuple[int, int]:
    ...
    # original skill's 44px) inflates subtitles on low-res videos — 44px on
    # a 270p clip covers ~16% of the frame height, 2.7x the intended share.
    is_portrait = video_height > video_width
    font_size = max(20, int(video_height * (0.042 if is_portrait else 0.061)))
...
def read_srt_lines(input_path: Path) -> list[dict]:
    ...
    content = Path(input_path).read_text(encoding="utf-8").strip()
    if not content:
...
def _vision_detector_binary() -> Path:
    ...
    if not source.exists():
        raise RuntimeError(
            f"Vision detector source not bundled: {source.name} (macOS-only feature)"
        )
    digest = hashlib.sha256(source.read_bytes()).hexdigest()[:12]
    binary = Path(tempfile.gettempdir()) / f"videotrans-face-detector-{digest}"
    if binary.exists():
        return binary
...
def burn_subtitles(video_path: Path, ass_path: Path, output_path: Path,
    ...
    Rotation is handled automatically by ffmpeg's built-in autorotate. It both
    physically corrects the frame orientation AND clears the Display Matrix in
    the output, so no manual transpose filter or metadata patching is needed.
    """
    ...
    if encoder == "videotoolbox":
        ...
        # Software x264: veryfast+crf20 is the measured sweet spot, and H264
        # is what video platforms prefer for uploads.
        video_codec = ["-c:v", "libx264", "-preset", "veryfast", "-crf", "20",
                       "-pix_fmt", "yuv420p", "-movflags", "+faststart"]
        log("x264 software encoding (veryfast, crf 20)")

    progress_path = Path(tempfile.gettempdir()) / "ffmpeg_burn_progress.txt"
    progress_path.unlink(missing_ok=True)

    cmd = [resolve_ffmpeg(), "-i", str(video_path)]
    ...
    log(f"Output: {Path(output_path).name} ({size_mb:.1f} MB)")


def _probe_video(video_path: Path) -> tuple[int, int, float, int]:
    """Return (display_width, display_height, duration, rotation)."""
    ...
    return video_w, video_h, video_duration, video_rotation
...
def run_burn(
    ...
    *,
    ...
    if transcript is None and srt_input is None:
        transcript = work_dir / "subtitle-transcript.json"
    if transcript is not None and not Path(transcript).exists():
        raise FileNotFoundError(f"Transcript not found: {transcript}")
    if srt_input is not None and not Path(srt_input).exists():
        raise FileNotFoundError(f"SRT not found: {srt_input}")
    progress_requested = resolve_progress_enabled(progress)
    if chapters is None and progress_requested:
        candidate = work_dir / "subtitle-chapters.json"
        chapters = candidate if candidate.exists() else None
    if chapters is not None and not Path(chapters).exists():
        raise FileNotFoundError(f"Chapters not found: {chapters}")

    output_path = Path(output) if output else video.with_name(video.stem + "_subtitled.mp4")
    ass_path = output_path.with_suffix(".ass")

    glossary_entries = load_user_glossary(glossary) if not srt_input else []
    if glossary_entries:
        set_display_replacements(glossary_entries)
        log(f"Loaded {len(glossary_entries)} glossary replacements")

    video_w, video_h, video_duration, video_rotation = _probe_video(video)
    log(f"Video display resolution: {video_w}x{video_h}"
        + (f" (stored rotated {video_rotation} deg)" if video_rotation else ""))
    ...
    if square_output:
        ...
        target_side = output_height if output_height and output_height > 0 else crop_side
    ...
    if srt_input:
        ...
        effective_max_chars = _resolve_effective_max_chars(max_chars, ass_w, ass_h, square_output)
    ...
    chapters_list = load_progress_chapters(chapters, video_duration, enabled=progress_requested)
    log(
    ...
    progress_overlay_height = (
        _progress_overlay_metrics(ass_h)[0] if chapters_list else 0
    )

    # Always write the SRT alongside the ASS/MP4 (README deliverable set);
    # --draft-output only overrides WHERE it lands.
    srt_path = Path(draft_output) if draft_output else output_path.with_suffix(".srt")
    ...
    if not no_beauty:
        ...
        camera_region = detect_camera_region(video, video_w, video_h)
    ...
    log(f"   ASS subtitle: {ass_path}")
    log(f"   SRT subtitle: {srt_path}")
    log("")
    log("Tip: Edit the .ass file to tweak font/size/position, then re-run.")
    return output_path


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="videotrans.burn",
        description=(
            "Burn reviewed subtitles into the final video. Run after the "
            "preview save; transcript and chapters are auto-discovered in "
            "<video>.subtitle-work/."
        ),
    )
    parser.add_argument("video", type=Path, help="Path to the exported video file (.mp4)")
    parser.add_argument("--transcript", type=Path, default=None,
                        help="Transcript to burn (default: <work-dir>/subtitle-transcript.json)")
    parser.add_argument("--srt-input", type=Path, default=None,
                        help="Burn a reviewed SRT directly without re-laying out text")
    parser.add_argument("--output", type=Path, default=None,
                        help="Output video path (default: <video-stem>_subtitled.mp4)")
    ...
    parser.add_argument("--chapters", type=Path, default=None,
                        help="Broad chapter JSON (default: <work-dir>/subtitle-chapters.json)")
    parser.add_argument("--progress", action=argparse.BooleanOptionalAction, default=None,
                        help="Enable chapter progress when chapters are available (default: on)")
    parser.add_argument("--max-chars", type=int, default=0,
                        help="Max visual chars per subtitle line. 0 = auto (landscape 28, portrait 16).")
    parser.add_argument("--draft-output", type=Path, default=None, help="Write an SRT draft for review")
    parser.add_argument("--draft-only", action="store_true", help="Only write the draft; no ASS or burn")
    parser.add_argument("--ass-only", action="store_true", help="Only generate the .ass file")
    ...
    parser.add_argument("--no-beauty", action="store_true",
                        help="Disable automatic face detection and the camera beauty pass.")
