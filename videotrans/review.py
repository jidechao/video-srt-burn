#!/usr/bin/env python3
"""Stage 2 — review ASR subtitles with Qwen and verify uncertain terms
against video frames.

Ported from the oil-subtitle skill's review_subtitles.py; the CLI-coupled
main() became a callable review_transcripts() so the pipeline can drive it
in-process.
"""
...
from videotrans.dashscope_client import call_qwen_multimodal_json, call_qwen_vision_json
...
def review_transcripts(
    *,
    ...
    """Review a transcript in place; returns the report dictionary."""
    video = Path(video)
    if not video.is_file():
        raise FileNotFoundError(f"Video not found: {video}")
    payload, segments = load_transcript(Path(transcript))

    log(f"Reviewing {len(segments)} subtitle segment(s) with {text_model}...")
    text_items, text_usage = model_candidates(segments, text_model, chunk_size)
    ...
    for item in candidates:
        ...
        if item["decision"] == "replace" and item["confidence"] >= text_confidence:
        ...
        frames = extract_frames(video, item["segment_id"], segment, frames_dir)
        ...
        if decision == "replace" and confidence >= vision_confidence:
            ...
            if changed:
                ...
                continue
        if decision == "keep" and confidence >= vision_confidence:
    ...
    report = {
        "source_transcript": str(Path(transcript).resolve()),
    ...
    log(
        f"Saved {output}; {len(applied)} applied/verified, "
